...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/dashboard/rg-regions'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/dashboard/rg-regions', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: RgRegions) {
        return item.id;
    }
    registerChangeInRgRegions() {
        this.eventSubscriber = this.eventManager.subscribe('rgRegionsListModification', (response) => this.loadAll());
    }
