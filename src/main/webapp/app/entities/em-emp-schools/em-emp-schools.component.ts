...
currentAccount: any;
    ...
    loadAll() {
        this.emEmpSchoolsService.query({
            page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/dashboard/em-emp-schools'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/dashboard/em-emp-schools', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
