...
@Component({
    selector: 'jhi-rg-qualifications',
...
currentAccount: any;
    ...
    loadAll() {
        this.rgQualificationsService.query({
            page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/rg-qualifications'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/rg-qualifications', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: RgQualifications) {
        return item.id;
    }
    registerChangeInRgQualifications() {
        this.eventSubscriber = this.eventManager.subscribe('rgQualificationsListModification', (response) => this.loadAll());
    }
