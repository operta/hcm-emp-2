...
currentAccount: any;
    ...
    loadAll() {
        this.ogWorkPlacesService.query({
            page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/dashboard/og-work-places'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/dashboard/og-work-places', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
