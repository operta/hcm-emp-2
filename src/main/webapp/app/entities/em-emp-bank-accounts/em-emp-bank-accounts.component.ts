...
currentAccount: any;
    ...
    loadAll() {
        this.emEmpBankAccountsService.query({
            page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/em-emp-bank-accounts'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/em-emp-bank-accounts', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
