...
currentAccount: any;
    ...
    loadAll() {
        this.emEmpSalariesService.query({
            page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/dashboard/em-emp-salaries'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/dashboard/em-emp-salaries', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
